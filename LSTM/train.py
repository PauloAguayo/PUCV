...
from prepare_data import OurDataset, randomizeDataset
...
def train_model():
    ...
    rndm_data = randomizeDataset(raw_data,true_label)
    rr = rndm_data.random_random()
    raw_data, true_label = rr[0], rr[1]
    ...
    for epoch in range(args['epochs']):
        ...
        print('Train Loss: {:6.4f}, Train Accuracy: {:6.2f}%, Time Consumption: {:6.2f} seconds;'.format(epoch_train_loss,epoch_train_acc*100, epoch_elapsed_time))
    ...
    with torch.no_grad():
        for enum,(batch_x_eval, batch_y_eval) in enumerate(eval_loader):
            ...
            if enum==0:
                outcomes_preds = eval_preds
                batch = batch_y_eval
            else:
                outcomes_preds = torch.hstack((outcomes_preds,eval_preds))
                batch = torch.hstack((batch,batch_y_eval))

        accuracy = accuracy_score(batch, outcomes_preds)
        f1 = f1_score(batch, outcomes_preds, average='macro')
        precision = precision_score(batch, outcomes_preds, average='macro')
        recall = recall_score(batch, outcomes_preds, average='macro')
