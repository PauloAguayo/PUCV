...
def format_training_data(pet_names, pet_labels, window, device=None):
    ...
    if device is None:
        y = torch.tensor(pet_labels_numeric)
        x = torch.tensor(pet_names_numeric)
    else:
...
class OurDataset(Dataset):
    def __init__(self, pet_names,pet_labels,window, device=None):
